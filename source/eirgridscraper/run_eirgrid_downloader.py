    ...
    if failed > 0:
        ...
        for area, result in results.items():
            if not result["success"]:
                logger.error(
                    f"  {area}: {result['error']} (method: {result['method']})"
                )
...
Regions: all, roi (Republic of Ireland), ni (Northern Ireland)
    ...
    date_group.add_argument(
        "--start", "-s", type=validate_date, help="Start date (YYYY-MM-DD)"
    )

    parser.add_argument("--end", "-e", type=validate_date, help="End date (YYYY-MM-DD)")
    ...
    parser.add_argument(
        "--output-dir", "-o", help="Output directory for data files (default: ./data)"
    )
    ...
    try:
        ...
        logger.info(
            f"File structure: data/{{metric}}/{{metric}}_{date_from}_{date_to}.json"
        )
        logger.info("=" * 60)

        # Create downloader
        headless = not args.show_browser
        downloader = UnifiedEirGridDownloader(
            data_dir=args.output_dir, headless=headless
        )
        ...
        for i, area in enumerate(areas, 1):
            ...
            if result["success"]:
                ...
                relative_path = (
                    Path(file_path).relative_to(Path("data"))
                    if "data" in file_path
                    else Path(file_path).name
                )
                logger.info(
                    f"  ✅ SUCCESS: {area} downloaded via {result['method']} → data/{relative_path}"
                )
            else:
        ...
        if successful == len(areas):
            ...
            logger.info(
                f"💾 Data organized in: data/{{metric}}/{{metric}}_{date_from}_{date_to}.json format"
            )
            return 0
        elif successful > 0:

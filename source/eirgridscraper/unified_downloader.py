...
try:
    from eirgridscraper.csv_downloader import CSVDataDownloader
...
class UnifiedEirGridDownloader:
    ...
    def __init__(self, data_dir: Optional[str] = None, headless: bool = True):
        ...
        self.csv_downloader = (
            CSVDataDownloader(data_dir=str(self.data_dir), headless=headless)
        ...
        self.stats = {"api_success": 0, "api_fail": 0, "csv_success": 0, "csv_fail": 0}
    ...
    def _generate_filename(
        self, area: str, date_from: str, date_to: str, region: str = None
    ) -> str:
    ...
    def download_area(
        ...
        self.logger.info(
            f"Starting download for {area} (region: {region}, dates: {date_from} to {date_to})"
        )
        ...
        if (
            include_forecast
            or force_scraping
            or area in self.csv_required_areas
            or (area in self.csv_preferred_areas and not force_scraping)
        ):
            ...
            if csv_downloader_available:
                method_reason = (
                    "forecast requested"
                    if include_forecast
                    else "force scraping" if force_scraping else "CSV preferred"
                )
                self.logger.info(
                    f"Using CSV download method for {area} ({method_reason})"
                )
                csv_result = self._download_via_csv(
                    area, region, date_from, date_to, include_forecast
                )
                ...
                if csv_result["success"]:
                    ...
                    self.logger.warning(
                        f"CSV download failed for {area}: {csv_result['error']}"
                    )
        ...
        if (
            area in self.api_areas
            and area not in self.csv_required_areas
            and "csv_download" not in result["method_attempted"]
        ):

            self.logger.info(f"Attempting API download for {area}")
            api_result = self._download_via_api(
                area,
                region.upper() if region.lower() != "all" else "ALL",
                date_from,
                date_to,
            )
        ...
        if (
            csv_downloader_available
            and "csv_download" not in result["method_attempted"]
            and area not in self.problematic_csv_areas
        ):
            ...
            if csv_result["success"]:
                ...
                self.stats["csv_fail"] += 1
                self.logger.warning(
                    f"CSV fallback failed for {area}: {csv_result['error']}"
                )

        # If we get here, all available methods failed
        result["error"] = (
            f"All available methods failed. Attempted: {', '.join(result['method_attempted'])}"
        )
        self.logger.error(f"All methods failed for {area}: {result['error']}")
        return result

    def _download_via_api(
        self, area: str, region: str, date_from: str, date_to: str
    ) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("Status") == "Success":
                        ...
                        if rows:
                            ...
                            self.logger.info(
                                f"API download successful: {point_count} data points"
                            )
                            return result
                        else:
                    ...
                    self.logger.warning(
                        f"Service unavailable, retrying in {2 ** attempt} seconds..."
                    )
                ...
                if attempt < max_retries - 1:
                    self.logger.warning(
                        f"API timeout, retrying in {2 ** attempt} seconds..."
                    )
                    time.sleep(2**attempt)
                    continue
            except Exception as e:
    ...
    def _download_via_csv(
        ...
        try:
            ...
            self.logger.debug(
                f"CSV download: {area}, duration={duration}, dates={date_from_fmt} to {date_to_fmt}"
            )
            ...
            if csv_result["success"]:
                ...
                actual_count = sum(
                    1 for point in ts if not point.get("is_forecast", False)
                )
                forecast_count = sum(
                    1 for point in ts if point.get("is_forecast", False)
                )
                self.logger.info(
                    f"CSV download successful: {len(ts)} total points ({actual_count} actual, {forecast_count} forecast)"
                )
            else:
                ...
                if result["debug_info"]:
                    self.logger.debug(
                        f"CSV download debug info: {result['debug_info']}"
                    )
    ...
    def _parse_api_data(self, rows: List[Dict], area: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "metadata": {"area": area, "total_points": len(time_series)},
        }
    ...
    def save_data(
        ...
        if target_file_path.exists() and update_existing:
            ...
            merged_data = self._merge_data(
                existing_data, data, area, region, date_from, date_to
            )
            ...
            overlapping_files = self._find_overlapping_files(
                metric_dir, area, date_from, date_to, region
            )

            if overlapping_files:
                self.logger.info(
                    f"Found {len(overlapping_files)} overlapping files for date range {date_from} to {date_to}"
                )
                ...
                for file_path in overlapping_files:
                    try:
                        ...
                        if len(filename_parts) >= 4:
                            ...
                            if (
                                file_region == region
                                and file_start == date_from
                                and file_end == date_to
                            ):
                ...
                if exact_match:
                    ...
                    self.logger.info(
                        f"Creating new file for date range {date_from} to {date_to}"
                    )
    ...
    def _merge_data(
        ...
        for new_point in new_ts:
            ...
            if normalized_new_time in existing_by_normalized_time:
                ...
                if old_point.get("is_forecast", False) and not new_point.get(
                    "is_forecast", False
                ):
                    ...
                    merged = True
                    self.logger.debug(
                        f"Replaced forecast with actual for {normalized_new_time}"
                    )

                # If old point was actual and new is forecast, add forecast_value but keep actual
                elif (
                    not old_point.get("is_forecast", False)
                    and new_point.get("is_forecast", False)
                    and "forecast_value" not in old_point
                ):
                    old_point["forecast_value"] = new_point.get("value")
                    merge_count += 1
                    merged = True
                    self.logger.debug(
                        f"Added forecast value to actual data for {normalized_new_time}"
                    )

                # Update if values are different and not downgrading from actual to forecast
                elif old_point.get("value") != new_point.get("value") and not (
                    not old_point.get("is_forecast", False)
                    and new_point.get("is_forecast", False)
                ):
                    # Keep the original time format from existing data for consistency
                ...
                if not merged:
                    self.logger.debug(
                        f"No merge needed for {normalized_new_time} (same value or downgrade)"
                    )
            else:
                # Add new time point (use consistent format)
                normalized_point = new_point.copy()
                normalized_point["time"] = self._normalize_time_format(
                    original_new_time
                )
        ...
        self.logger.info(
            f"Data merge complete: {merge_count} merged, {update_count} updated, {add_count} added"
        )
        ...
        existing["data"]["extracted_at"] = new_data.get(
            "extracted_at", datetime.now().isoformat()
        )
...
def test_new_file_structure():
    ...
    result = downloader.download_area(
        area="co2_intensity", region="all", date_from="2025-06-23", date_to="2025-06-23"
    )

    if result["success"]:
        # Save with new structure
        file_path = downloader.save_data(
            result["data"], "co2_intensity", "all", "2025-06-23", "2025-06-23"
        )

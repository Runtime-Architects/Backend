...
try:
    from eirgridscraper.unified_downloader import UnifiedEirGridDownloader
...
class ComprehensiveEirGridTester:
    ...
    def __init__(
        self, headless: bool = True, debug: bool = False, data_dir: str = None
    ):
        self.debug = debug
        ...
        self.downloader = (
            UnifiedEirGridDownloader(data_dir=self.data_dir, headless=headless)
    ...
    def test_api_area(
        self, area: str, api_area: str, region: str = "ALL", date_str: str = None
    ) -> Dict:
    ...
    def test_csv_area(
        self, area: str, region: str = "all", date_str: str = None
    ) -> Dict:
        ...
        try:
            ...
            csv_result = self.downloader._download_via_csv(
                area, region, date_str, date_str, False
            )
            ...
            if csv_result["success"]:
                ...
                result["actual_points"] = sum(
                    1 for p in time_series if not p.get("is_forecast", False)
                )
                result["forecast_points"] = (
                    result["data_points"] - result["actual_points"]
                )
            else:
            ...
            result["response_time"] = (
                round(time.time() - start_time, 2) if "start_time" in locals() else None
            )
        ...
        try:
            ...
            if unified_result["success"]:
                ...
                try:
                    ...
                    expected_filename = (
                        f"{area}_{date_str}_{date_str}.json"
                        if region.lower() == "all"
                        else f"{area}_{region}_{date_str}_{date_str}.json"
                    )
                    expected_path = expected_dir / expected_filename

                    result["file_structure_correct"] = (
                        Path(file_path) == expected_path and expected_path.exists()
                    )
    ...
    def test_file_structure(self):
        ...
        for i, (area, region, date_from, date_to) in enumerate(test_scenarios, 1):
            print(
                f"[{i}/{len(test_scenarios)}] Testing file structure: {area} ({region}) {date_from} to {date_to}"
            )
            ...
            try:
                ...
                if structure_correct:
                    print(
                        f"   ✅ SUCCESS - Correct structure: {expected_path.relative_to(Path(self.data_dir).parent)}"
                    )
                else:
    ...
    def run_comprehensive_test(self):
        ...
        print("=" * 80)
        print(
            f"📅 Testing with yesterday's date: {(datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')}"
        )
        ...
        for i, (area, api_area) in enumerate(self.api_areas.items()):
            ...
            if result["success"]:
                print(
                    f"   ✅ SUCCESS - {result['data_points']} points ({result['response_time']}s)"
                )
            else:
        ...
        for i, area in enumerate(self.csv_areas):
            ...
            if result["success"]:
                print(
                    f"   ✅ SUCCESS - {result['data_points']} points ({result['actual_points']} actual, {result['forecast_points']} forecast) ({result['response_time']}s)"
                )
            else:
                print(f"   ❌ FAILED - {result['error']} ({result['response_time']}s)")
        ...
        for i, area in enumerate(key_test_areas):
            ...
            if result["success"]:
                file_status = (
                    "✅ correct structure"
                    if result["file_structure_correct"]
                    else "⚠️ incorrect structure"
                )
                relative_path = (
                    Path(result["file_path"]).relative_to(Path(self.data_dir).parent)
                    if result["file_path"]
                    else "unknown"
                )
                print(
                    f"   ✅ SUCCESS - Used {result['actual_method_used']} method, {result['data_points']} points ({result['response_time']}s)"
                )
    ...
    def print_comprehensive_summary(self):
        ...
        for area, result in self.results["api_tests"].items():
            if result["success"]:
                print(
                    f"      🟢 {area:<20} - {result['data_points']:>3} points ({result['response_time']}s)"
                )
        ...
        if self.results["csv_tests"]:
            csv_success = sum(
                1 for r in self.results["csv_tests"].values() if r["success"]
            )
            ...
            for area, result in self.results["csv_tests"].items():
                if result["success"]:
                    print(
                        f"      🟢 {area:<20} - {result['data_points']:>3} points ({result['actual_points']} actual, {result['forecast_points']} forecast)"
                    )
        ...
        if self.results["unified_tests"]:
            unified_success = sum(
                1 for r in self.results["unified_tests"].values() if r["success"]
            )
            unified_total = len(self.results["unified_tests"])

            print(f"\n🎯 UNIFIED METHOD RESULTS:")
            print(
                f"   Success Rate: {(unified_success/unified_total*100):.1f}% ({unified_success}/{unified_total} areas)"
            )
            ...
            for area, result in self.results["unified_tests"].items():
                ...
                if result["success"] and "file_structure_correct" in result:
                    file_status = (
                        " 📁✅" if result["file_structure_correct"] else " 📁⚠️"
                    )
        ...
        if self.results["file_structure_tests"]:
            structure_success = sum(
                1
                for r in self.results["file_structure_tests"].values()
                if r.get("success", False) and r.get("structure_correct", False)
            )
            structure_total = len(self.results["file_structure_tests"])

            print(f"\n📁 FILE STRUCTURE TEST RESULTS:")
            print(
                f"   Success Rate: {(structure_success/structure_total*100):.1f}% ({structure_success}/{structure_total} tests)"
            )
            ...
            for test_name, result in self.results["file_structure_tests"].items():
                if result.get("success", False):
                    ...
                    relative_path = (
                        Path(result["file_path"]).relative_to(
                    ...
                    print(
                        f"      ❌ {test_name:<30} - {result.get('error', 'Unknown error')}"
                    )
        ...
        for area in self.csv_areas:
            api_status = (
                "✅"
                if self.results["api_tests"].get(area, {}).get("success", False)
                else "❌"
            )
            csv_status = (
                "✅"
                if self.results["csv_tests"].get(area, {}).get("success", False)
                else "❌"
            )
            unified_status = (
                "✅"
                if self.results["unified_tests"].get(area, {}).get("success", False)
                else "❌"
            )
            ...
            if area in self.results["unified_tests"]:
                if self.results["unified_tests"][area].get(
                    "file_structure_correct", False
                ):
                    file_status = "✅"
            ...
            print(
                f"   {area:<20} {api_status}    {csv_status}    {unified_status}      {file_status}"
            )
        ...
        if self.results["file_structure_tests"]:
            structure_success = sum(
                1
                for r in self.results["file_structure_tests"].values()
                if r.get("structure_correct", False)
            )
            structure_total = len(self.results["file_structure_tests"])
            structure_success_rate = (
                (structure_success / structure_total * 100)
        ...
        if structure_success_rate == 100:
            ...
            print(
                "   📁 File organization needs attention - check directory structure."
            )
    ...
    def test_specific_area(self, area: str):
        ...
        for date_name, date_str in dates_to_test:
            ...
            # Test API
            api_result = self.test_api_area(
                area, self.api_areas[area], date_str=date_str
            )
            api_status = "✅" if api_result["success"] else "❌"
            api_info = api_result.get(
                "error", f"{api_result.get('data_points', 0)} points"
            )
            ...
            if area in self.csv_areas:
                ...
                csv_info = csv_result.get(
                    "error", f"{csv_result.get('data_points', 0)} points"
                )
            ...
            unified_info = unified_result.get(
                "error", f"{unified_result.get('data_points', 0)} points"
            )

            file_info = ""
            if unified_result["success"] and unified_result.get("file_path"):
                file_status = (
                    "✅" if unified_result.get("file_structure_correct", False) else "⚠️"
                )
                relative_path = Path(unified_result["file_path"]).relative_to(
                    Path(self.data_dir).parent
                )
                file_info = f" → {relative_path} {file_status}"

            print(
                f"   Unified: {unified_status} Used {method_used} ({unified_info}){file_info}"
            )
...
Examples:
  ...
  python comprehensive_test.py --data-dir ./test_organized_data
    ...
    parser.add_argument(
        "area", nargs="?", help="Specific area to test in detail (optional)"
    )

    parser.add_argument("--debug", action="store_true", help="Enable debug logging")

    parser.add_argument(
        "--show-browser", action="store_true", help="Show browser during CSV tests"
    )

    parser.add_argument("--data-dir", help="Test data directory (default: test_data)")

    args = parser.parse_args()

    # Initialize tester
    tester = ComprehensiveEirGridTester(
        headless=not args.show_browser, debug=args.debug, data_dir=args.data_dir
    )

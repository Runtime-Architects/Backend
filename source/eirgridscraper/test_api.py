...
def quick_validation_test():
    ...
    if result1["success"]:
        ...
        if method == "api":
            ...
            logger.warning(
                f"   ⚠️  Used {method} instead of API (API might have failed)"
            )
    else:
    ...
    if result2["success"]:
        ...
        actual = sum(
            1 for p in result2["data"]["time_series"] if not p.get("is_forecast", False)
        )
    ...
    api_used_appropriately = (
        ...
        result1["success"]
        and result1.get("method") == "csv_download"
        and stats["api_fail"] > 0
    )

    # Check if CSV was used when requested
    csv_used_when_needed = (
        result2["success"] and result2.get("method") == "csv_download"
    ) and (result3["success"] and result3.get("method") == "csv_download")
    ...
    if api_used_appropriately and csv_used_when_needed:
        ...
        logger.warning(
            "⚠️  PARTIAL: API prioritization works, but CSV flags not respected"
        )
    elif csv_used_when_needed:
        logger.warning("⚠️  PARTIAL: CSV flags work, but API prioritization failed")
    else:
        logger.error(
            "❌ VALIDATION FAILED: Method prioritization not working as expected"
        )

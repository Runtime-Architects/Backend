...
class OrganizedDataMerger:
    ...
    def merge_files(
        ...
        if output_path:
            ...
            if merged_date_range:
                output = self._update_filename_with_date_range(
                    base_path, merged_date_range
                )
            else:
                output = base_path
    ...
    def _merge_two_datasets(self, base: Dict, new: Dict) -> Dict:
        ...
        for new_point in new_ts:
            ...
            if time_key in merged_by_time:
                ...
                if existing.get("is_forecast", False) and not new_point.get(
                    "is_forecast", False
                ):
                ...
                elif existing.get("value") != new_point.get("value") and not (
                    not existing.get("is_forecast", False)
                ...
                if "forecast_value" in new_point and "forecast_value" not in existing:
                    merged_by_time[time_key]["forecast_value"] = new_point[
                        "forecast_value"
                    ]
    ...
    def _calculate_merged_date_range(
        self, merged_data: Dict
    ) -> Optional[Tuple[str, str]]:
        """Calculate the date range of merged data"""

        data_section = (
            merged_data.get("data", {}) if "data" in merged_data else merged_data
        )
    ...
    def _update_filename_with_date_range(
        self, original_path: str, date_range: Tuple[str, str]
    ) -> str:
    ...
    def find_metric_files(
        self, data_dir: str, area: str = None
    ) -> Dict[str, List[str]]:
        """
    ...
    def clean_overlapping_files(
        self, data_dir: str, area: str = None, dry_run: bool = True
    ):
        """
            ...
            for i, file1 in enumerate(files):
                for j, file2 in enumerate(files[i + 1 :], i + 1):
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"Error checking overlap between {file1} and {file2}: {e}"
                        )

            if overlaps_found:
                self.logger.info(
                    f"  🔍 Found {len(overlaps_found)} overlapping file pairs:"
                )
                ...
                for file1, file2, range1, range2 in overlaps_found:
                    ...
                    self.logger.info(
                        f"    📄 {file1_name} ({range1[0]} to {range1[1]})"
                    )
                    self.logger.info(
                        f"    📄 {file2_name} ({range2[0]} to {range2[1]})"
                    )
                    ...
                    if not dry_run:
                        ...
                        try:
                            merged_path = self.merge_files(
                                [base_file, merge_file], base_file
                            )
                            ...
                            if merge_file != merged_path:
                                ...
                                self.logger.info(
                                    f"    🗑️  Removed {Path(merge_file).name}"
                                )
    ...
    def _extract_date_range_from_filename(
        self, file_path: str
...
Examples:
  ...
  python data_merger.py --find-overlaps ./data co2_intensity 2025-06-20 2025-06-25
    ...
    parser.add_argument(
        "--clean-dir", help="Clean overlapping files in organized directory structure"
    )

    parser.add_argument("--area", help="Specific area to process when cleaning")

    parser.add_argument("--analyze", help="Analyze a data file")
    ...
    try:
        if args.analyze:
            ...
            overlapping = merger.find_overlapping_files(
                data_dir, area, date_from, date_to
            )
            ...
            print("=" * 60)
            merger.clean_overlapping_files(
                args.clean_dir, args.area, dry_run=not args.execute
            )

...
carbon_agent = AssistantAgent(
            name="CarbonAgent", model_client=client, tools=[emission_tool, daily_analyzer_tool, 
...
""" async def main() -> None:
    async with LocalCommandLineCodeExecutor(work_dir="coding") as executor:
        ...
        await Console(
            agent.run_stream(
                task=f"Based on last weeks data, What is the best time to use my appliances today in Ireland?"
            )
        )
